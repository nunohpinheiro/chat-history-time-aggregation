...
public class ChatHistoryInfluxDbFixture : IAsyncLifetime
{
    public const string TestBucketName = "service-api-integration-tests-bucket";
    public const string TestOrganizationName = "service-api-integration-tests-org";
    ...
    private static async Task EnsureCreateTestBucket(string organizationId)
    {
        ...
        if (await bucketsApi.FindBucketByNameAsync(TestBucketName) is null)
            await bucketsApi.CreateBucketAsync(new Bucket(
                ...
                {
                    new BucketRetentionRules(everySeconds: 86400 * 365 * 10) // 10 years
                }));
    }
